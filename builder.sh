...
make O=out ARCH=arm64 vendor/fog-perf_defconfig

make -j12 ARCH=arm64 SUBARCH=arm64 O=out \
        CC="clang --target=aarch64 -march=armv8-a+crc+crypto -mcpu=cortex-a73+crc+crypto -mtune=cortex-a73 -funroll-loops -O3"\
        AR="llvm-ar" \
	NM="llvm-nm" \
	LD="ld.lld" \
